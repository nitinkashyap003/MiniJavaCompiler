...
public class ParserLogic {
	...
	public ParserLogic(FileReader file) {
		lexerLogic = new LexerLogic(file);
		token = lexerLogic.getToken(); // prime the pump
	}
	
	// verifies current token type and grabs next token or reports error
	private boolean eat(TokenType type) {
		if (token.getType() == type) {
			token = lexerLogic.getToken();
			return true;
		} else {
			error(type);
			return false;
	...
	private Identifier parseIdentifier() {
		...
		if (token.getType() == TokenType.ID)
			id = new Identifier(lexerLogic.getIdVal());
	...
	private Statement parseStatement() {
		...
		if (token.getType() == TokenType.ID) {
			Identifier id = new Identifier(lexerLogic.getIdVal());
			eat(TokenType.ID);
			...
			if (id.getName().equals("System") && token.getType() == TokenType.DOT) {
				...
				if (token.getType() == TokenType.ID && lexerLogic.getIdVal().equals("out"))
					eat(TokenType.ID);
				else {
				...
				if (token.getType() == TokenType.ID && lexerLogic.getIdVal().equals("println"))
					eat(TokenType.ID);
				else {
					eat(TokenType.STATEMENT);
					return null;
	...
	private Exp parsePrimaryExp() {
		switch (token.getType()) {
			...
			case INT_CONST:
				int value = lexerLogic.getIntVal();
		...
		while (token.getType() != TokenType.RETURN && token.getType() != TokenType.EOF) {
			...
			switch (token.getType()) {
				...
				case ID:
					...
					if (lexerLogic.peek().getType() == TokenType.ID) {
						vars.addElement(parseVarDecl());
						break;
					}
